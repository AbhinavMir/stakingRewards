  ...
  ReentrancyGuard
{
  ...
  function getRewardForDuration() external view override returns (uint256) {
    return
      rewardPerToken()
        .sub(userRewardPerTokenPaid[msg.sender])
        .mul(rewardRate)
        .mul(1e24)
        .div(periodFinish)
...
contract StakingRewardsFactory is Ownable {
  ...
  address public rewardsTokenFactory;
    ...
    Ownable()
  {
    ...
    rewardsTokenFactory = _rewardsToken;
  ...
  function deploy(
    ...
    info.stakingRewards = address(
      new StakingRewards(address(this), rewardsTokenFactory, stakingToken)
    );
  ...
  function claimRewardAmount(address stakingToken) public {
    ...
    if (info.rewardAmount > 0 && info.duration > 0) {
      ...
      require(
        IERC20(rewardsTokenFactory).transfer(info.stakingRewards, rewardAmount),
